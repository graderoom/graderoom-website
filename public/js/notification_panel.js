    ...
    function createChangelogNotifications(_versionNames, _versionDates, _versionMessages) {
        ...
        for (; i < _versionNames.length; i++) {
            ...
            if (version.startsWith("Stable")) {
                ...
                createDismissed(version, "Beta", version, changelogMessage(version, _versionMessages[i]), false, false, isBeta, _versionDates[i], false);
            } else if (version.startsWith("Announcement")) {
    ...
    function createDonationProgressNotification() {
        if (!sessionStorage.getItem("donoNotifCounted")) {
            sessionStorage.setItem("donoNotifCounted", "1");
            let count = parseInt(localStorage.getItem("donoNotifCount") ?? "2") + 1;
            localStorage.setItem("donoNotifCount", count);
        }
        let count = parseInt(localStorage.getItem("donoNotifCount") || "0");
        let dismissedAt = parseInt(localStorage.getItem("donoNotifDismissedCount") || "-1");
        let showActive = count % 7 === 0 && dismissedAt !== count;

        $.ajax("/donationProgress", {
            method: "GET",
            success: function (data) {
                if (!data || data.donorCount <= 0) return;
                let supporterLine = `<div style="font-size:0.85rem;margin-bottom:0.5rem"><i class="fa fa-heart" style="color:#e74c3c;margin-right:0.3rem"></i>${data.donorCount} ${data.donorCount === 1 ? "person has" : "people have"} supported Graderoom</div>`;
                let progressBar = ``;
                if (data.effectiveGoal > 0) {
                    let pct = Math.min(100, Math.round((data.thisMonthReceived / data.effectiveGoal) * 100));
                    let raised = data.thisMonthReceived.toFixed(2);
                    let goal = data.effectiveGoal.toFixed(2);
                    let remaining = Math.max(0, data.effectiveGoal - data.thisMonthReceived).toFixed(2);
                    progressBar = `
                        <div style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:0.4rem">
                            <span style="font-size:1.05rem;font-weight:700">${pct}%<span style="font-size:0.75rem;font-weight:400;opacity:0.55;margin-left:0.25rem">funded</span>&nbsp;</span>
                            <span style="font-size:0.78rem;opacity:0.6">$${raised} / $${goal}</span>
                        </div>
                        <div style="height:5px;background:rgba(128,128,128,0.2);border-radius:3px;overflow:hidden">
                            <div style="height:100%;width:${pct}%;background:linear-gradient(90deg,#f90,#ff5722);border-radius:3px;transition:width 0.6s ease"></div>
                        </div>
                        <div style="font-size:0.72rem;opacity:0.5;margin-top:0.3rem;text-align:right">$${remaining} remaining this month</div>
                    `;
                }
                let message = supporterLine + progressBar + `<span class="notification-button" onclick="showCard('#settingsCardDisplay'); openTab(0)"><b><i class="fa fa-external-link-square"></i> Donate</b></span>`;
                let hasActiveGoal = data.effectiveGoal > 0;
                let dismissible = !hasActiveGoal || !!data.userHasDonated;
                createNotification("dono-progress", "Announcement", "Support Graderoom", message, dismissible, !showActive, false, showActive, hasActiveGoal && !dismissible && showActive, Date.now(), true, true);
                // Hook dismiss button to persist dismissal until next instance
                if (showActive) {
                    let notif = notifications.unread.find(n => n.id === "dono-progress") || notifications.pinned.find(n => n.id === "dono-progress");
                    if (notif?.ref) {
                        notif.ref.querySelector(".notification-dismiss")?.addEventListener("click", () => {
                            localStorage.setItem("donoNotifDismissedCount", count);
                        });
                    }
                }
            }
